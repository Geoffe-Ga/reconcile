...
class ProposalView(discord.ui.View):
    def __init__(
        self, store: ReconcileStore, group_name: str, doc_id: int, prop_id: int
    ) -> None:
    ...
    async def _vote(self, interaction: discord.Interaction, choice: str) -> None:
        ...
        if not group or interaction.user.id not in group.members:
            await interaction.response.send_message(
                "Only group members may vote on proposals.", ephemeral=True
            )
            return
        err = self.store.record_vote(
            self.group_name, self.doc_id, self.prop_id, interaction.user.id, choice
        )
        if err:
...
class VoteView(discord.ui.View):
    ...
    async def _cast(self, interaction: discord.Interaction, delta: int):
        ...
        if not rec or rec.closed or rec.cancelled:
            await interaction.response.send_message(
                "This vote is closed.", ephemeral=True
            )
            return
        ...
        if not sides:
            await interaction.response.send_message(
                "You are not eligible to vote in this reconcile.", ephemeral=True
            )
            return
        if len(sides) > 1:
            ...
            for s in sides:
                ...
                view.add_item(b)
            await interaction.response.send_message(
                "Choose which hat to wear for this vote:", view=view, ephemeral=True
            )
            return
        # Single side
        err = self.store.record_reconcile_vote(
            self.rid, interaction.user.id, sides[0], delta
        )
        if err:
            await interaction.response.send_message(err, ephemeral=True)
        else:
            await interaction.response.send_message(
                f"Vote recorded: {delta:+d}", ephemeral=True
            )
...
def reconcile_embed(store: ReconcileStore, rid: int) -> discord.Embed:
    ...
    if rec.closed or rec.cancelled:
        state = (
            "CANCELLED" if rec.cancelled else ("PASSED" if rec.passes() else "FAILED")
        )
...
class DocumentView(discord.ui.View):
    ...
    async def propose(
        ...
        try:
            ...
            await interaction.response.send_message(
                "Failed to load document.", ephemeral=True
            )
            return
        if not doc:
            await interaction.response.send_message(
    ...
    async def view_props(
        ...
        for pid, p in sorted(proposals.items()):
            lines.append(
                f"• #{pid} by <@{p.author_id}> — {'MERGED' if p.merged else 'OPEN'}"
            )
        txt = "\n".join(lines)

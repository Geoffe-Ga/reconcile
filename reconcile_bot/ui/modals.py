...
class DocumentModal(discord.ui.Modal, title="Create Document"):
    def __init__(
        self, store: ReconcileStore, group_name: str, title_text: str, tags: list[str]
    ) -> None:
        super().__init__()
        self.store = store
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        ...
        await interaction.response.send_message(
            f"Document `{self.title_text}` "
            f"created in `{self.group_name}` "
            f"and preview posted to {docs_ch.mention}.",
            ephemeral=True,
        )
...
class ProposalModal(discord.ui.Modal, title="Propose Edit"):
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        prop_id = self.store.add_proposal(
            self.group_name, self.doc_id, interaction.user.id, self.proposal_input.value
        )
        if prop_id is None:
            await interaction.response.send_message(
                "Failed to submit proposal.", ephemeral=True
            )
            return
        await interaction.response.send_message(
            f"Proposal `{prop_id}` submitted for document "
            f"`{self.doc_id}` in group `{self.group_name}`.",
            ephemeral=True,
        )

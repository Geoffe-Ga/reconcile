...
def register_commands(bot: commands.Bot, store: ReconcileStore) -> None:
    ...
    async def create_group(
        interaction: discord.Interaction, name: str, description: str, tags: str
    ) -> None:
    ...
    @tree.command(
        name="view_document", description="View a document and interact with it"
    )
    ...
    async def view_document(
        ...
        if not document:
            await interaction.response.send_message(
                "Document not found.", ephemeral=True
            )
            return
    ...
    async def recommend(interaction: discord.Interaction) -> None:
        ...
        if not names:
            await interaction.response.send_message(
                "No recommendations available yet.", ephemeral=True
            )
            return
        embed = discord.Embed(
    ...
    async def my_groups(interaction: discord.Interaction) -> None:
        ...
        if not groups:
            await interaction.response.send_message(
                "You are not in any groups yet. Use `/join_group`.", ephemeral=True
            )
            return
    ...
    def reconcile_picker_view(store: ReconcileStore, guild_id: int) -> discord.ui.View:
        ...
        async def refresh(inter):
            ...
            other_groups = [
                g.name for g in store.groups.values() if member_id not in g.members
            ]
            your_group.options = [
    ...
    async def start_reconcile_flow(
        ...
        if (
            not mode
            or (mode == "group_vs_group" and (not your_group or not target_group))
        ...
        # TODO: ensure channels exist: this should be a ensure_channels() call
        _, votes_id = "", ""
        rid = store.create_reconcile(
            ...
            (
                ...
                else f"solo:{interaction.user.display_name}"
            ),
            target_group,
        ...
        view = VoteView(
            store, rid, store.get_reconcile(rid).a_side, store.get_reconcile(rid).b_side
        )
    ...
    async def reconcile_status(
        ...
        if not rec:
            ...
            return
        await interaction.response.send_message(
            embed=reconcile_embed(store, reconcile_id), ephemeral=True
        )
    ...
    async def reconcile_cancel(
        ...
        # allow manage_guild or opener
        # (we did not save opener; keep simple: require manage guild for now)
        if not interaction.user.guild_permissions.manage_guild:
            await interaction.response.send_message(
                "Only a server manager can cancel for now.", ephemeral=True
            )
            return
        ok = store.cancel_reconcile(reconcile_id)
        if not ok:
            await interaction.response.send_message(
                "Reconcile not found.", ephemeral=True
            )
            return

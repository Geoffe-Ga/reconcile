...
class Reconcile:
    ...
    thread_id: int | None  # thread id if created
    message_id: int | None  # id of the tally message with buttons
    created_ts: float
    close_ts: float
    closed: bool = False
    ...
    def side_for_member(
        self, user_id: int, groups: dict[str, Group]
    ) -> list[str] | None:
        ...
        if self.is_group_vs_group():
            ...
            if a and user_id in a.members:
                sides.append(self.a_side)
            if b and user_id in b.members:
                sides.append(self.b_side)
        else:
